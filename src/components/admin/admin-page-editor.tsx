...
import { updatePage, type Page } from '@/firebase/services/pages';
import {
  Dialog,
  DialogContent,
  DialogHeader,
                ...
                <FormField control={form.control} name="imageurl" render={({ field }) => (
                    <FormItem>
                      ...
                      <AdminImageUploadField
                        ...
                        folder="college-portal/blocks/hero"
                      />
                      <FormMessage />
                    </FormItem>
                )} />
                ...
                <FormField control={form.control} name="imageUrl" render={({ field }) => (
                    <FormItem>
                      ...
                      <AdminImageUploadField
                        ...
                        folder="college-portal/blocks/image"
                      />
                      <FormMessage />
                    </FormItem>
                )} />
...
export function AdminPageEditor({ page, open, onOpenChange, onUpdate }: { page: Page; open: boolean; onOpenChange: (open: boolean) => void; onUpdate: () => void }) {
  ...
  const pageDetailsForm = useForm<PageFormValues>({
    resolver: zodResolver(pageFormSchema),
  });

  const fetchBlocks = async () => {
    setIsLoading(true);
    const blocksData = await getBlocksAdmin(page.id);
  ...
  const handleAddBlock = async (type: Block['type']) => {
    ...
    try {
        await addBlock({ page_id: page.id, type, order_position: newOrder, visible: true, data: defaultData });
  ...
  const onSubmitPageDetails = async (values: PageFormValues) => {
    setIsSubmitting(true);
    try {
        await updatePage(page.id, values);
        toast({ title: 'Success', description: 'Page details saved successfully.' });
        onUpdate();
        onOpenChange(false);
    } catch(e) {
         toast({ variant: 'destructive', title: 'Error', description: 'Failed to save page details.' });
    }
    setIsSubmitting(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Editing Page: {page.title}</DialogTitle>
        </DialogHeader>
        
        <div className="flex-grow overflow-y-auto pr-4 -mr-6 pl-1 py-4">
            <Form {...pageDetailsForm}>
                <form id="page-details-form" onSubmit={pageDetailsForm.handleSubmit(onSubmitPageDetails)} className="space-y-4">
                    <FormField control={pageDetailsForm.control} name="title" render={({ field }) => (
                    <FormItem><FormLabel>Page Title</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={pageDetailsForm.control} name="slug" render={({ field }) => (
                    <FormItem><FormLabel>Slug</FormLabel><FormControl><Input {...field} disabled={page.slug === 'home'} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={pageDetailsForm.control} name="description" render={({ field }) => (
            ...
            <div className="mt-6">
                <h3 className="text-lg font-medium mb-2">Page Blocks</h3>
                {isLoading ? (
        ...
        <DialogFooter className="pt-4 border-t mt-auto">
            <DialogClose asChild><Button type="button" variant="ghost">Cancel</Button></DialogClose>
            <Button type="submit" form="page-details-form" disabled={isSubmitting}>
              {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Save Changes
            </Button>
        </DialogFooter>
